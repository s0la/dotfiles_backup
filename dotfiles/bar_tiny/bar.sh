...
bg='d073858c'	# d266747e solid background d2505f67 d340495 9548605a e55A6373
solid_bg='e55A6373'
...
while read -r line; do
	...
	media="  %{T5}$progressbar  %{F#b3b3b3}%{T3}$controls%{O15}$song_info %{F#c5c5c5}%{T6}$media_volume$buttons$media_ending"
	workspaces="%{c}%{F#777}$workspaces"
	#usage="$usage_beggining %{T6}$mount $disk%{O5}%{T6}$cpu%{O20}$ram%{F#c2c2c2}%{O7}"
	#usage="%{T6} $disk%{O10}%{F#c2c2c2}"
	system_info="%{r}$usage$system_info_beggining %{F#c5c5c5}$mail%{T6}$volume%{O30}$date"
	echo -e "$media $workspaces $system_info $shut_down"
done < "$fifo" | lemonbar -p -B#$bg -f "$font1" -o -2 \
				    -f "$font2" -o -4 \
				    -f "$font3" -o -5 \
				    -f "$font4" -o -0 \
				    -f "$font5" -o -4 \
				    -f "$font6" -o -2 \
				    f "$font7" -o -3 -a 70 -g x20 | bash 
